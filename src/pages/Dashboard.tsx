...
export default function Dashboard() {
  ...
  const handleAddFundsToGoal = (goalId, fundAmount) => {
    ...
    const success = addFundsToGoal(goalId, amount);
    if (success) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 pb-12">
      ...
      <div className="card p-6 space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-white/40">Set a monthly amount to see when you'll reach your goal</p>
        </div>
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {savings && savings.length > 0 ? (
            savings.map((s) => {
              ...
              return (
                <div key={s.id} className="p-3 rounded-lg bg-white/5 border border-white/10 group hover:bg-white/10 transition">
                  ...
                  <p className="text-sm font-semibold">€{s.amount.toFixed(2)} / €{s.target.toFixed(2)}</p>
                  <div className="mt-2 w-full bg-white/10 rounded-full h-1.5">
                    <div
                      className="bg-gradient-to-r from-purple-500 to-pink-500 h-1.5 rounded-full transition-all"
                      style={{ width: `${Math.min((s.amount / s.target) * 100, 100)}%` }}
                    />
                  </div>
                  {monthsRemaining !== null && (
                    <p className="text-xs text-white/50 mt-2">
                      {monthsRemaining === 0 ? '✓ Goal achieved!' : `${monthsRemaining} month${monthsRemaining !== 1 ? 's' : ''} remaining`}
                    </p>
                  )}
                  <div className="mt-3 space-y-2">
                    ...
                    {fundError[s.id] && <p className="text-xs text-red-400">{fundError[s.id]}</p>}
                    <div className="flex gap-2">
                      <input
                        type="number"
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {transactions && transactions.length > 0 ? (
            transactions.slice(0, 10).map((t) => (
              <div key={t.id} className="flex justify-between items-center p-3 rounded-lg bg-white/5 border border-white/10 group hover:bg-white/10 transition">
