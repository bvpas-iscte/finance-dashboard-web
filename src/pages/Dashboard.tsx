...
export default function Dashboard() {
  ...
  const updateGoalMonthly = useFinanceStore((s) => s.updateGoalMonthly);
  ...
  const [goalFunds, setGoalFunds] = useState<Record<number, string>>({});
  const [goalMonthly, setGoalMonthly] = useState<Record<number, string>>({});
  const [fundError, setFundError] = useState<Record<number, string>>({});
  ...
  const handleAddFundsToGoal = (goalId: number, fundAmount: string) => {
    if (!fundAmount || parseFloat(fundAmount) <= 0) return;

    const amt = parseFloat(fundAmount);

    if (amt > balance) {
      setFundError({ ...fundError, [goalId]: 'Insufficient balance' });
      return;
    }

    const success = addFundsToGoal(goalId, amt);

    if (success) {
      setGoalFunds({ ...goalFunds, [goalId]: '' });
      setFundError({ ...fundError, [goalId]: '' });
    }
  };

  const calculateMonthsToGoal = (goal: SavingsGoal) => {
    if (!goal.monthlyContribution || goal.monthlyContribution <= 0) return null;
    const remaining = goal.target - goal.amount;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 pb-12">
      ...
      {/* BALANCE CARD */}
      <div className="lg:col-span-4 bg-black border border-white/10 rounded-3xl backdrop-blur-xl p-8 space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 p-4 rounded-xl bg-white/5 border border-white/10">
            ...
            <p className="text-lg font-semibold">€{expenses.toFixed(2)}</p>
          </div>
          <div className="flex-1 p-4 rounded-xl bg-white/5 border border-white/10">
            <p className="text-xs text-white/60">Savings</p>
            <p className="text-lg font-semibold">€{totalSavings.toFixed(2)}</p>
          </div>
        </div>
      </div>

      {/* CHART */}
      <div className="lg:col-span-2 bg-black border border-white/10 rounded-2xl backdrop-blur-xl p-6">
      ...
      {/* SAVINGS GOALS */}
      <div className="bg-black border border-white/10 rounded-2xl backdrop-blur-xl p-6 space-y-4">
        <div className="space-y-2">
          <h3 className="text-sm font-semibold">Savings Goals</h3>
          <p className="text-xs text-white/40">
            Set a monthly amount to see when you'll reach your goal
          </p>
        </div>
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {savings.length > 0 ? (
            savings.map((s) => {
              const monthsRemaining = calculateMonthsToGoal(s);
              return (
                <div
                  key={s.id}
                ...
                >
                  ...
                  <p className="text-sm font-semibold">
                    €{s.amount.toFixed(2)} / €{s.target.toFixed(2)}
                  </p>
                  <div className="mt-2 w-full bg-white/10 rounded-full h-1.5">
                    <div
                      className="bg-gradient-to-r from-purple-500 to-pink-500 h-1.5 rounded-full transition-all"
                      style={{
                        width: `${Math.min((s.amount / s.target) * 100, 100)}%`,
                      }}
                    />
                  </div>
                  ...
                  <div className="mt-3 space-y-2">
                    {/* FIX: min-w-0 on the flex container prevents input from overflowing */}
                    <div className="flex gap-2 min-w-0">
                      <input
                        type="number"
                        placeholder="Add funds"
                        value={goalFunds[s.id] || ''}
                        onChange={(e) => {
                          setGoalFunds({ ...goalFunds, [s.id]: e.target.value });
                          setFundError({ ...fundError, [s.id]: '' });
                        }}
                        className="min-w-0 flex-1 px-2 py-1 rounded text-xs bg-white/5 border border-white/10 text-white placeholder-white/30"
                      />
                      <button
                        onClick={() => handleAddFundsToGoal(s.id, goalFunds[s.id])}
                        className="flex-shrink-0 px-3 py-1 rounded text-xs bg-purple-600/50 hover:bg-purple-600 text-white transition whitespace-nowrap"
                      >
                        Add
                      </button>
                    </div>
                    {fundError[s.id] && (
                      <p className="text-xs text-red-400">{fundError[s.id]}</p>
                    )}
                    <div className="flex gap-2 min-w-0">
                      <input
                        type="number"
                        placeholder="Monthly amount"
                        value={goalMonthly[s.id] || ''}
                        onChange={(e) => {
                          setGoalMonthly({ ...goalMonthly, [s.id]: e.target.value });
                          updateGoalMonthly(s.id, e.target.value);
                        }}
                        className="min-w-0 flex-1 px-2 py-1 rounded text-xs bg-white/5 border border-white/10 text-white placeholder-white/30"
                      />
                    </div>
      ...
      {/* RECENT TRANSACTIONS */}
      <div className="lg:col-span-2 bg-black border border-white/10 rounded-2xl backdrop-blur-xl p-6 space-y-4">
